 ...
 * 
 * Tests verify ERROR logs are emitted when executor is missing or fails.
 */
...
describe("BUG-002: Error Logging for Executor Failures (P1)", () => {
  ...
  it("BUG-002: ERROR log when executor is undefined", async () => {
    ...
    // Should have ERROR log stating executor not configured
    const hasExecutorError = consoleErrors.some(msg =>
      msg.includes("ERROR") || msg.includes("executor") || msg.includes("undefined")
    );
    expect(hasExecutorError).toBe(true);

    // Poll event should have reason
    const pollEvent = events.find(e => e.type === "scheduler.poll");
    expect(pollEvent?.payload?.reason).toBeDefined();
  });

  it("BUG-002: ERROR log on executor spawn failure", async () => {
    const task = await store.create({
      title: "Test task",
      body: "Body",
  ...
  it("BUG-002: ERROR log includes actionable reason", async () => {
    ...
    // Error should include actionable context
    const errorEvent = events.find(e => e.type === "dispatch.error");
  ...
  it("BUG-002: ERROR log with stack trace on exception", async () => {
    const task = await store.create({
      title: "Test task",
      body: "Body",
      routing: { agent: "test-agent" },
    ...
    executor.spawn = vi.fn().mockRejectedValue(new Error("Spawn exception with stack"));
    ...
    // Should have error event with exception message
    const errorEvent = events.find(e => e.type === "dispatch.error");
  ...
  it("BUG-002: acceptance - ERROR logs for all failure modes", async () => {
    // Test all failure scenarios produce ERROR logs
    
    // Scenario 1: Executor undefined
    ...
    // Scenario 2: Executor spawn fails
