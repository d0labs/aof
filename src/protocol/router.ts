...
function buildCompletionReason(opts: { outcome: RunResult["outcome"]; notes?: string; blockers?: string[] }): string | undefined {
...
function buildStatusReason(payload: StatusUpdatePayload): string | undefined {
...
function shouldAppendWorkLog(payload: StatusUpdatePayload): boolean {
  return Boolean(payload.progress || payload.notes || (payload.blockers && payload.blockers.length > 0));
}

function buildWorkLogEntry(payload: StatusUpdatePayload): string | null {
...
function appendSection(body: string, title: string, lines: string[]): string {
